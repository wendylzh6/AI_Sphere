...
export default function CareerMobility({ personId }: CareerMobilityProps) {
  ...
  return (
    <div className="bg-[#090A10]/95 backdrop-blur-xl border border-white/10 rounded-xl shadow-2xl px-3 py-2 pointer-events-auto">
      ...
      {!history || history.length === 0 ? (
        ...
        <div className="flex items-start gap-0 overflow-x-auto career-timeline-scroll pb-0.5">
          ...
          {[...history].reverse().map((step, i, arr) => {
            ...
            return (
              <div key={`${step.y}-${step.c}-${i}`} className="flex flex-col items-center relative" style={{ flex: 1, minWidth: 68 }}>
